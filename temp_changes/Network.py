...
class Network:
    ...
    def train(self, games, vGames, p):
        ...
        for epoch in range(epochs):
            ...
            random.shuffle(games)
            batches = network_helper.toBatches(games, p['batchSize'])
            vBatches = network_helper.toBatches(vGames, p['batchSize'])
            #chunkedGames = network_helper.toBatchChunks(games, bs, numCPUs)

            #   Baseline calculation of cost on training, validation data
            if epoch == 0:
                self.tCosts.append(self.totalCost(batches, p))
                self.vCosts.append(self.totalCost(vBatches, p))

            for i in range(numBatches):
                z, zNorm, a = self.ff_track(batches[i][0])
                partials = self.backprop(z, zNorm, a, batches[i][1], p)
                ...
                self.last_dC_dbias[0] = partials[3][0].reshape((-1,1)) + p['mom'] * self.last_dC_dbias[0]
                self.biases[0] -= nu * self.last_dC_dbias[0]
                            
                for lay in range(len(self.layers)):
                    self.last_dC_dw[lay] = partials[0][lay] + p['mom'] * self.last_dC_dw[lay]
                    self.weights[lay] -= nu * self.last_dC_dw[lay]

                    #   Batch norm-related parameters
                    if lay < len(self.layers) - 1 and lay not in self.downSampLays:
                        self.last_dC_db[lay] = partials[1][lay].reshape((-1,1)) + p['mom'] * self.last_dC_db[lay]
                        self.beta[lay] -= p['batchNormScale'] * nu * self.last_dC_db[lay]

                        self.last_dC_dg[lay] = partials[2][lay].reshape((-1,1)) + p['mom'] * self.last_dC_dg[lay]
    ...
    def backprop(self, z, zNorm, a, y, p):
        ...
        assert y.shape[1] == p['batchSize'], "incorrect batch size"
        ...
        for i in range(len(z)):
            ...
            dC_dw[-1-i] = np.dot(dC_dz[-1-i], a[-2-i].T) / p['batchSize'] + p['weightDec'] * self.weights[-1-i]

        return [dC_dw, dC_db, dC_dg, dC_dbias]

    #   Given the entire set of training examples, returns the average cost per example.
    def totalCost(self, batches, p):
        return sum([self.batchLoss(b) for b in batches]) / len(batches)
    ...
    def setPopStats(self, batches, p):
        ...
        for b in batches:
            statList = network_helper.get_pop_stats(self, b)
            for lay in range(len(self.layers)):
                if lay not in self.downSampLays:
                    popMean[lay] += statList[0][lay] / numBatches
                    popVar[lay] += statList[1][lay] / numBatches
